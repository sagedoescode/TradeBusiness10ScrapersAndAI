...
def export(niche, state, max_retries=5, initial_retry_delay=5, max_retry_delay=60):
    ...
    for row in csv_data:
        ...
        if 'shop' in new_row[3].lower():
            print(new_row[3])
            continue
        if 'store' in new_row[3].lower():
            print(new_row[3])
            continue
        if 'supplier' in new_row[3].lower():
            print(new_row[3])
            continue
...
def create_filtered_state_sheet_if_not_exists(sheet, SPREADSHEET_ID, state):
    sheet_metadata = sheet.get(spreadsheetId=SPREADSHEET_ID).execute()
    sheets = sheet_metadata.get('sheets', '')
    sheet_names = [s['properties']['title'] for s in sheets]
    state = state.capitalize() + " Filtered"
    if state not in sheet_names:
        state_sheet_id = next((s['properties']['sheetId'] for s in sheets if s['properties']['title'] == 'State'), None)
        if state_sheet_id is not None:
            request = {'destinationSpreadsheetId': SPREADSHEET_ID}
...
def export_filtered(niche, state, max_retries=5, initial_retry_delay=5, max_retry_delay=60):
    SCOPES = ['https://www.googleapis.com/auth/spreadsheets']
    SPREADSHEET_ID = '1ILnlUYCMQl1gdfqRQM0PtB-N2d5QDynBQ9Rdeewcq4U'
    creds = service_account.Credentials.from_service_account_file(
        'creds/verdant-coyote-430500-j4-ef1fad885b23.json', scopes=SCOPES)
    ...
    service = build('sheets', 'v4', credentials=creds)
    sheet = service.spreadsheets()
    create_filtered_state_sheet_if_not_exists(sheet, SPREADSHEET_ID, state)

    # Get existing data
    state_F = state.capitalize() + " Filtered"
    existing_data = sheet.values().get(spreadsheetId=SPREADSHEET_ID, range=f'{state_F}!A:C').execute().get('values', [])
    existing_business_names = set(row[2] for row in existing_data if len(row) > 2)
    ...
    if new_data:
        ...
        RANGE_NAME = f'{state_F}!A{len(existing_data) + 1}'
        batch_update_values_request_body = {
            'value_input_option': 'RAW',
            'data': [
                {
